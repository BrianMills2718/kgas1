...
The analyze document-pipeline investigation identifies `CompleteGraphRAGPipeline.process_document()` / `execute_complete_pipeline()` as the likely real backing path. Because that path is file-path-oriented and currently relies on T01 support for `.pdf` and `.txt`, the recommended implementation slice is a narrow `.txt` adapter and fixture before broader upload support. A runtime probe of that `.txt` slice currently blocks at `ServiceManager.identity_service` because Neo4j credentials/connection are unavailable. [18]
