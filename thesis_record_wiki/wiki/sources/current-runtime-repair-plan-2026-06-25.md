...
The analyze document-pipeline investigation identifies `CompleteGraphRAGPipeline.process_document()` / `execute_complete_pipeline()` as the real backing path for the first `/api/analyze` repair. Because that path is file-path-oriented and currently proven for `.txt`, `/api/analyze` now wires only `.txt` uploads through a temp-file adapter, serializes the real complete-pipeline output into `AnalysisResponse`, and keeps `.pdf`/`.docx`/`.doc`/`.md` at explicit 501 until those paths are separately proven. With the existing local Neo4j Docker container and `NEO4J_PASSWORD` in the shell environment, both the complete-pipeline smoke test and a skip-safe live API test execute the `.txt` path through real stages, extract relationships, create Neo4j edges, verify Neo4j integration, and report `end_to_end_success=True`. The key adapter repair was grouping current T23A `chunk_ref` entities for T27 instead of looking only for historical `source_ref`. The graph-connectivity caveat is now classified: full connected-component analysis is not computed during request-time validation because the prior `CALL { ... }` variable-length traversal can hang on the accumulated graph; graph validation now uses a bounded summary and preserves entity/edge verification as the proof basis. [16][18][19][20][21]
