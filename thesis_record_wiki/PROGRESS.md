...
1. Next recommended step: install/manage `en_core_web_sm` in the isolated environment if dependency parsing is in scope, then add a small analysis-agent end-to-end relationship propagation test.
2. Follow-up audit: check direct T27 callers outside `AnalysisAgent` for the same T23A/T27 entity-contract boundary.
