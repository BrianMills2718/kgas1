...
- `PENDING` installed and declared `en-core-web-sm==3.8.0`, added a runtime model-availability test, and verified T27 can load the shared spaCy model without the previous missing-model error.

## Next

1. Next recommended step: audit direct T27 callers outside `AnalysisAgent` for the same T23A/T27 entity-contract boundary.
