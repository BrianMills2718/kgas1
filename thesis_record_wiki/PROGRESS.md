...
- Pending commit: theoretical-exploration proposal-materials slice, including dissertation framing, academic guidance, validation critiques/matrix, HSPC pointer, worked examples, safety framework, deprecated concepts, and no-literal-key finding.

## Next

1. Run wiki lint, commit, and push the theoretical-exploration proposal-materials slice.
2. Next recommended step: ingest `archive/theoretical_exploration/archived_proposal_materials/full_example_architecture/` as the next bounded full-example/uncertainty/DAG slice.
